...
class RotDPGV(object):
    """
    BBP module implementation of rotd50 PGV provided by UCB.
    Rotd50 inputs seismograms and outputs response spectra
    """
    @staticmethod
    def do_rotd_pgv(workdir, peer_input_e_file, peer_input_n_file,
    ...
    def run(self):
        ...
        self.log = os.path.join(install.A_OUT_LOG_DIR, str(sim_id),
                                "%d.rotdpgv_%s.log" % (sim_id, sta_base))
        ...
        for site in site_list:
            ...
            bbp_formatter.bbp2peer(bbpfile, out_n_acc, out_e_acc, out_z_acc, accel=False)

            # Let's have rotD50 create these output files
            out_rotd_base = "%d.%s.rdpgv" % (sim_id, stat)
            tmp_rotd = os.path.join(a_tmpdir, out_rotd_base)
            out_rotd = os.path.join(a_outdir, out_rotd_base)

            # Run the rotD50 program
            self.do_rotd_pgv(a_tmpdir, out_e_acc, out_n_acc, out_z_acc,
                           out_rotd, self.log)
        ...
        # All done!
        print("RotDPGV Completed".center(80, '-'))

if __name__ == '__main__':
    print("Testing Module: %s" % (os.path.basename(sys.argv[0])))
    ME = RotDPGV(sys.argv[1], sim_id=int(sys.argv[2]))
    ME.run()
    sys.exit(0)
