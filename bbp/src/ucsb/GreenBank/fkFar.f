...
c      read(*,*)mb,srclayer,src
      ...
      if (mb.gt.nlay) then
	 write(*,'(a)') 'Number of layers exceeds maxium #'
	 stop 
      endif
      hs = 0.
c      write(*,'(a)') ' layer thick   Vp     Vs     rho   Qa    Qb'
      do i = 1, mb
c         read(*,*)d(i),a(i),b(i),rho(i),qa(i),qb(i)
c	 write(*,'(i4,4f7.2,2e9.2)')i,d(i),a(i),b(i),rho(i),qa(i),qb(i)
	 if (i .LT. srclayer) hs = hs + d(i)
      enddo
      vs = b(srclayer)
      write(*,'(a15,f8.1,a20,i3)') 'source depth =',hs,
...
c      read(*,*) sigma,nfft,dt,taper,tb,smth
	 ...
	 write(*,'(a)') 'Number of points exceeds maximum allowed'
	 stop 
      endif
      dw = pi2/(nfft*dt)
      sigma = sigma*dw/pi2
...
c      read(*,*) nx
      if (nx.gt.ndis) then
	 write(*,'(a)') 'Number of ranges exceeds max.'
	 stop 
      endif
      xmax = r_max
      dk = dk*pi/xmax
...
c     write(*,'(a3,f9.5,a8,f9.2,a8,i9)')'dk',dk,'kmax',kc,'N',total
      write(*,1001)'dk =',dk,'kmax =',kc,'pmax =',pmax,'N =',total
...
c changed by pcl, 10/29/2004
	 ...
	 do i=1,n		! start k-loop
            call kernel(k,u)
c           write(*,'(f6.4,2e12.4)') k,real(u(1,1)),real(u(1,2))
	    do ix=1,nx
               z = k*x(ix)
...
c n=2
	    ...
	    if ( mod(count,tenPerc) .EQ. 0) then
	       write(*,'(i4,a6)') int(100.*count/total)+1, '% done'
	    endif
	 enddo			! end of k-loop
	 lowpass = 1.
	 ...
	 omega = omega + dw
      ...
      write(*,'(i9,a40)') count,' 100% done, writing files ... '
