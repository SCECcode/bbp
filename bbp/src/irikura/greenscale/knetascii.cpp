...
void knetascii::get_data(vector<int>& data) const
{
	int	i, n;
	n	= data_.size();
...
void knetascii::get_data(vector<double>& data) const
{
	int	i, n;
	n	= data_.size();
	if( data.size() != data_.size() ){
...
void knetascii::set_data(  const vector<double>& data )
{
	double absmin = fabs( *std::min_element( data.begin(), data.end() ) );
	double absmax = fabs( *std::max_element( data.begin(), data.end() ) );
	denominator_ = ( absmax > absmin ) ? absmax : absmin;
	...
	for(int i=0;i<n;i++) data_[i] = (int)(data[i]/denominator_*numerator_);
}
...
istream& operator>>(istream& strm, knetascii& knet)
{
	...
	for(i=0;i<HEADER_LINE_NUMBER;i++){
		...
		case 12:
			...
			break;
		case 13:
			//n	= strData.find('/'); (cm/s)のとき使用できない
			//knet.set_denominator(atof((strData.substr(0,n-1)).c_str()));
			//knet.set_numerator(atof((strData.substr(n+1)).c_str()));
			n1	= strData.find('(');
			n2	= strData.find(')');
			knet.set_denominator(atof((strData.substr(0,n1)).c_str()));
			knet.set_numerator(atof((strData.substr(n2+2)).c_str()));
			break;
		case 14:
