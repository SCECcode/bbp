...
c 20220409: apply alphaT to rvf to increase rupture speed for dipping/reverse faults
...
c RWG 2022-10-26
c
c   The following lines starting with "c ###-" have been replaced
c   with the block starting at "c START 2022-10-26" and ending at
c   "c END 2022-10-26". The new code fixes a potential bug in the
c   DO loop because the k2,kend limits cover np2+1 indices
c   instead of np2.
c

c ###- CFFF the following 9 lines are new (moved from before)
c ###-        if(vr.le.0.0) then
c ###-           ratim = rupt(iv,i,j)
c ###-        else
c ###-           xra = shyp(iv) - (i - 0.5*(nx(iv)+1))*dx(iv)
c ###-           yra = dhyp(iv) - (j - 0.5)*dw(iv)
c ###-           ratim=SQRT(xra**2+yra**2)/VR
c ###-
c ###-           if(irand.gt.0) then
c ###-              ratim=ratim+(rand_numb(0)-0.5)*0.1*ratim
c ###-           endif
c ###-        endif
c ###-
c ###-        kst=int(ratim/dt) + int(sub_tstart/dt)
c ###- CFFF end
c ###-
c ###-        do 1722 k=1,nsum
c ###-             si=rand_numb(0)
c ###- c            dris=(float(k)-1.0+si)*rise/dt
c ###-             dris=si*rise/dt
c ###-
c ###-             k2=nint(dris)
c ###-             if(nsum.eq.1) k2=0
c ###-
c ###- CFFF the following 10 lines are modified
c ###-
c ###-        k2 = k2 + kst
c ###-        kend = k2 + np2
c ###-        if(kend.gt.ndata) kend = ndata
c ###-
c ###-        print*,'k2= ',k2,sub_tstart,ratim
c ###-
c ###- C  RWG 04/20/04
c ###- C  remove scaling by 'ratio', now done in stoc_f() using bigC
c ###-
c ###-        DO 1733 li=k2,kend
c ###-             DS(1,li) =DS(1,li)+sddp(iv,i,j)*stdd(li-k2,1)
c ###-             DS(2,li) =DS(2,li)+sddp(iv,i,j)*stdd(li-k2,2)
c ###-             DS(3,li) =DS(3,li)+sddp(iv,i,j)*stdd(li-k2,3)
c ###- 1733     CONTINUE
c ###-
c ###- 1722     CONTINUE
c ###-
...
c   Call to rand_numb(0) not needed for calculation, but incrementing
c   random number sequence makes result backward compatible with
c   previous versions
       si=rand_numb(0)

c   Change kst to make sure it rounds to the closest index and is >0
       kst=int((ratim + sub_tstart)/dt + 0.5)
       if(kst.lt.1) kst=1

c   Since nsum is hardwired to be '1', we can remove
c   the "do 1722" loop

       kend = kst + (np2 - 1)
       if(kend.gt.ndata) kend = ndata

       print*,'kst= ',kst,sub_tstart,ratim

       DO 1733 li=kst,kend
         DS(1,li) = DS(1,li)+sddp(iv,i,j)*stdd(li-(kst-1),1)
         DS(2,li) = DS(2,li)+sddp(iv,i,j)*stdd(li-(kst-1),2)
         DS(3,li) = DS(3,li)+sddp(iv,i,j)*stdd(li-(kst-1),3)
1733     CONTINUE
