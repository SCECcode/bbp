...
void uncert(int,int,float *,float *,float *,float *,float *,float *);
...
void uncert(int nv,int np,float *res,float *b,float *sig,float *sig0,float *m90,float *p90)
{
int i, j;
float invn, invn1, ttfac;

invn = 1.0/(float)(nv);
invn1 = 1.0/(float)(nv-1);

if(nv > 56)
   ttfac = 1.64*sqrt(invn);
else
   ttfac = t95[nv-2]*sqrt(invn);
...
for(j=0;j<np;j++)
   {
   b[j] = b[j]*invn;
   sig[j] = sqrt(invn1*(m90[j] - nv*b[j]*b[j]));      /* corrected for bias */
