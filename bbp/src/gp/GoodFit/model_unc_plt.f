...
c     Prepare mplot input files of the residuals
      call biasplt ( misfit, bias, npts, per, freq, sigmabias, ieq, 1)

c     Compute the mean residual
      call meanResid (nsta,nper,misfit,bias,sigma,sigmaZero,sigmaBias)
...
c     Compute the 90% confidence interval of the mean using the T distribution
      call confInter ( CL90, bias, sigma, nsta, nper )
      
c     Prepare mplot input files of the model bias
      call biasplt ( misfit, bias, nper, per, freq, CL90, ieq, 0)

c     Plot the se of a single observation
      call varplt ( nper, per, freq, sigma, sigmaZero )
...
c -------------------------------------------------------------
	...
	print *,'nper,bias,sigma,sigmaZero,sigmaMean:'
      do j=1,nper
        bias(j) = sum(j) / nsta
        sigma(j) = sqrt( ( sum2(j) - nsta*bias(j)**2 ) / ( nsta-1 ) ) 
        sigmaZero(j) = sqrt( sum2(j) / nsta )
...
c ---------------------------------------------------------
      ...
      subroutine confInter ( CL90, bias, sigma, n, nper )
...
c ---------------------------------------------------------
      ...
      subroutine biasplt ( misfit, bias, npts, per, freq, CL90, ieq, flag)
...
c --------------------------------------------------------------------
      ...
      subroutine varplt ( nper, per, freq, sigma, sigmaZero )
      real per(1), freq(1)
